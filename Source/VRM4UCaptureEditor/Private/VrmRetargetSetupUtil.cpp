...
namespace
{
	...
	// VRM humanoid-name -> VRoid model bone name. Only used when a mesh has neither a
...
namespace
{
	...
	UIKRigDefinition* FindOrCreateIKRig(USkeletalMesh* Mesh, const FString& Folder, TArray<FString>& Messages)
	{
		...
		else
		{
			...
			Log(Messages, bBuilt
				...
				: FString::Printf(TEXT("FAILED: engine auto-characterizer does not recognize '%s'. Build the IK rig manually (docs/ik-retargeter-asset-setup.md) and re-run."), *Mesh->GetName()));
		}
	...
	// — the exact "arms ride ~45 degrees low" failure. Create + activate an auto-aligned
	// pose; "Default Pose" stays untouched for rollback.
