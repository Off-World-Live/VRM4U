# VMC → LiveLink ARKit face bridge (implemented 2026-07-03)
...
- **`UVrmRetargetSetupUtil::SetupVMCFaceForActor(Actor, SubjectName)`** — the wizard step
  ...
  (found by name via the asset registry; an existing LiveLink-consuming face ABP is kept) and
...
- **Scene lint** (three new checks, only on MetaHuman-face components whose sibling body is
  ...
  (LiveLink ABP but no publisher on the actor — fine if an iPhone drives it), and
...
present in every UE install, zero user burden. The actual client comes from the engine-bundled
