...
The same setup is scriptable from editor Python if you want to batch or automate it:
```python
import unreal
...
```
New assets are marked dirty, not saved, so save them yourself. Nothing existing is modified
...
From Python: `unreal.VrmRetargetSetupUtil.setup_vmc_face_for_actor(actor, "VMCFace")`.

## The lint: "VRM4U: Lint VMC Scene" (Tools menu)

Checks every skeletal mesh component in the level. Each finding carries a fix hint.
Also scriptable from Python: `unreal.VrmSceneLint.lint_world_and_log(world)` (returns the
finding count, with details in `LogVRM4UCaptureEditor`).
